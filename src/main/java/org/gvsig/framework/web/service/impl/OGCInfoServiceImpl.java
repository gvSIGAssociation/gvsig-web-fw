...
public class OGCInfoServiceImpl implements OGCInfoService {
    ...
    public WMTSInfo getCapabilitiesFromWMTS(String urlServerWMTS,
            List<String> listCrs, boolean useCrsSelected)
            throws ServerGeoException {
        ...
        try {
            ...
            String tileMatrixSelectedId = null;
                ...
                if (!listCrs.isEmpty()) {
                    if (useCrsSelected) {
                        if (listCrs.contains(supportedCRS)) {
                            tileMatrixSelectedId = identifier;
                        }
                    }
                    else {
            ...
            // supported
            // or crs is null
      if (tileMatrixSelectedId != null || listCrs.isEmpty()) {
                // get and set layers
                WMTSThemes layerListAsThemes = wmtsClient
          ...
          if(!listCrs.isEmpty()){
            List<WMTSTileMatrixSetLink> tileMatrixSetLink = layer.getTileMatrixSetLink();
              ...
              if(tileMatrixSelectedId.equals(wmtsLink.getTileMatrixSetId())){
                                crsSupported = true;
                            }
                        }
                    }
          if(crsSupported || listCrs.isEmpty()){

(function() {
	L.HistoryControl = L.Control
			.extend({
				options : {
					position : 'topright',
					maxMovesToSave : 10, // set to 0 for unlimited
					useExternalControls : false, // set to true to hide
													// buttons on map and use
													// your own. Can still use
													// goBack, goForward, and
													// allow this to take care
													// of storing history.
				...
				onAdd : function(map) {
					...
					if (!this.options.useExternalControls) {
						this._backButton = this._createButton('back',
								container, this.goBack, this);
						this._forwardButton = this._createButton('forward',
								container, this.goForward, this);
					}
				...
				moveWithoutTriggeringEvent : function(zoomCenter) {
					...
					this.performActionWithoutTriggeringEvent(function() {
						_this._map.setView(zoomCenter.centerPoint,
								zoomCenter.zoom);
					});
				},
				goBack : function() {
					return this._invokeBackOrForward('historyback',
							this._state.history, this._state.future);
				},
				goForward : function() {
					return this._invokeBackOrForward('historyforward',
							this._state.future, this._state.history);
				},
				...
				_createButton : function(name, container, action, _this) {
					...
					L.DomEvent.on(button, 'click', stop).on(button,
							'mousedown', stop).on(button, 'dblclick', stop).on(
							button, 'click', L.DomEvent.preventDefault).on(
							button, 'click', action, _this).on(button, 'click',
							this._refocusOnMap, _this);
				...
				_updateDisabled : function() {
					...
					if (backDisabled !== this._state.backDisabled) {
						...
						this._map.fire('historyback'
								+ (backDisabled ? 'disabled' : 'enabled'));
					}
					if (forwardDisabled !== this._state.forwardDisabled) {
						this._state.forwardDisabled = forwardDisabled;
						this._map.fire('historyforward'
								+ (forwardDisabled ? 'disabled' : 'enabled'));
					}
					if (!this.options.useExternalControls) {
						this._setButtonDisabled(this._backButton, backDisabled);
						this._setButtonDisabled(this._forwardButton,
								forwardDisabled);
					}
				},
				...
				_invokeBackOrForward : function(eventName, stackToPop,
						stackToPushCurrent) {
					var response = this._popStackAndUseLocation(stackToPop,
							stackToPushCurrent);
					if (response) {
				...
				_popStackAndUseLocation : function(stackToPop,
						stackToPushCurrent) {
					// check if we can pop
					if ($.isArray(stackToPop.items)
							&& stackToPop.items.length > 0) {
						var current = this
								._buildZoomCenterObjectFromCurrent(this._map);
						// get most recent
							...
							.on(
									...
									function(e) {
										if (!_this._state.ignoringEvents) {
											var current = _this
													._buildZoomCenterObjectFromCurrent(e.target);
											if (_this.options
													.shouldSaveMoveInHistory(current)) {
												_this._state.future.items = [];
												_this._push(
														_this._state.history,
														current);
											}
										}
